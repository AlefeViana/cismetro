...
function Footer() 
{
	...
	$this->MultiCell(187, 4,"CASO NÃO POSSA COMPARECER, FAVOR AVISAR O MAIS RÝPIDO POSSÓVEL PARA QUE OUTRO PACIENTE POSSA SER BENEFICIADO COM O AGENDAMENTO NESSA VAGA.", 0, 'C');
	$this->ln(1);
...
$pdf->ln();
if($l['cdendc'] > 0)
	$pdf->Row(array('ENDEREÇO: '.S::create($l['Logradouro'])->titleize(["de", "da", "do"]).', Nº: '.$l['crednumero'].', Bairro: '.S::create($l['Bairro'])->titleize(["de", "da", "do"]).', Cidade: '.S::create($l2['NmCidade'])->titleize(["de", "da", "do"]).' - SP'),1);
else
	$pdf->Row2(array('ENDEREÇO: '.$l['logradouro'].', N°: '.$l['Numero'].', BAIRRO: '.$l['Bairro'].', CIDADE: '.$l2['NmCidade'].' - SP'),1);
$pdf->ln();
$pdf->Row2(array('DATA DA CONSULTA / EXAME: '.$DtAgCons.'   HORÝRIO '.$HoraAgCons),1);
$pdf->ln();
...
$pdf->Ln();
$pdf->Row(array('DECLARO QUE ME FORAM APRESENTADAS AS OPÇÕES DE LOCAIS PARA A REALIZAÇÃO 		DA CONSULTA/ PROCEDIMENTO/ EXAME DENTRO DOS PRESTADORES CREDENCIADOS JUNTO AO CISMETRO, 		RESPEITANDO A LOGÝSTICA DO MUNICÝPIO.'),1);
/*
$pdf->Ln();
$pdf->Ln();
$pdf->SetX(60);
