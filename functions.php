...
function renewSession($db, int $userId){
	...
	//Validação acrescentada 22/06/2022 - solicitação reunião com Jorge
	$sql2 = "UPDATE tbusuario SET UltimoLogin = NOW() WHERE CdUsuario = ".$userId;
...
function createSession($db, array $params) : bool
{
	...
	//Validação acrescentada 22/06/2022 - solicitação reunião com Jorge
	$sql2 = "UPDATE tbusuario SET UltimoLogin = NOW() WHERE CdUsuario = ".$params["user_id"];
	$query1 = mysqli_query($db,$sql2);
...
function setAuthSession(array $user, $db)
{

	$tmp_session_sql = "SELECT cfg.valor FROM tbconfig cfg WHERE cfg.tipo = 'tempodesessao'";
	$qry = mysqli_query($db,$tmp_session_sql);
	...
	$_SESSION['cdunidade'] = $user['cdunidade'];
	$_SESSION['cdgestor'] = $user['cdgestor'];
	$_SESSION['exp_length'] = $result;
	$_SESSION['exp'] = time() + $_SESSION['exp_length'];
	$_SESSION['cdpaciente'] = $user['cdpaciente'];
    $_SESSION["token"] = jwtCreate(
        [
            'cdCliente' => CONNECTION_ID, 
...
function mascaraCNPJ($valor) {
    return mascara(str_pad($valor, 14, '0', STR_PAD_LEFT), "##.###.###/####-##");
}
