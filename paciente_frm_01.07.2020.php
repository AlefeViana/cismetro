...
<?php
	...
	if(($_GET['id']) && ($_GET['acao']=='edit') || ($_GET['acao']=='del'))
	{
			...
			$acao = "edit";
	  		...
	  		$sql = "SELECT p.Status as status,CdPaciente,p.CdBairro,NmPaciente,RG,CPF,csus,p.CertidaoMatricula,
...
<div class="row justify-content-center">
    ...
    <div class="col-md-8">
		...
		<div class="row">
				...
				<div class="form-group">
					...
					<?php 
						...
						while ($logradouroData = mysqli_fetch_array($query)){
							echo '<option '.($row["cdlogr"] == $logradouroData["cdlogr"] ? "selected" : "").' value="'.$logradouroData["cdlogr"].'" selected="selected">'.S::create($logradouroData["logra"])->titleize(["de", "da", "do"]).' - '.$logradouroData["cdlogr"].'</option>';	
						}
					?>
					</select>
				</div>
			...
			<button type="submit"
			class='btn <?php if($_GET['acao'] == "del") echo "btn-danger"; elseif($_GET['acao'] == "edit") echo "btn-primary"; else echo "btn-success";?>'  
			name="btn" 
