<?php
	...
	sc.Obs, ac.obs,ep.cdsus, Pa.NmProcedimento, sc.Obs1, p.DtNasc, p.csus, sc.idcombo,p.Celular
	...
	//ValidaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o para envio do sms
...
//fazemos a inclusÃƒÆ’Ã‚Â£o do arquivo com a classe FPDF
require('relatorios/fpdf/fpdf.php');

class PDF extends FPDF
{

###### CÃƒÆ’Ã¢â‚¬Å“DIGO DE BARRAS ######
...
function draw_code39($code, $x, $y, $w, $h) 
{
	...
	###### FIM DO CÃƒÆ’Ã¢â‚¬Å“DIGO DE BARRAS ######

function Header() 
{
	$this->Sety(5);
	if ($this->header1 == 1){
		$this->Image('img/logo.jpg',5,3,208,40,'jpg');  
	}
	$this->Ln(10);
}
...
var  $i=0;
   ...
   //MÃƒÆ’Ã‚Â©todo Footer que estiliza o rodapÃƒÆ’Ã‚Â© da pÃƒÆ’Ã‚Â¡gina
function Footer() 
{
	...
	$this->MultiCell(187, 4,"ATEN��O: OBRIGAT�RIO APRESENTA�AO DO CART�O SUS, PEDIDO DO EXAME E/OU ENCAMINHAMENTO GUIA DE AUTORIZA�AO PARA REALIZAR O ATENDIMENTO.", 0, 'C');
	$this->MultiCell(187, 4,"CASO N�O POSSA COMPARECER, FAVOR AVISAR O MAIS R�PIDO POSS�VEL PARA QUE OUTRO PACIENTE POSSA SER BENEFICIADO COM O AGENDAMENTO NESSA VAGA.", 0, 'C');
	$this->ln(1);
	
	/*$this->MultiCell(187, 6, 'CISEVMJ - CIS ENTRE OS VALES DO MUCURI E JEQUITINHONHA - TEÃƒÆ’Ã¢â‚¬Å“FILO OTONI - MG 
	          RUA SANTOS DUMONT, NÃƒâ€šÃ‚Âº 30, CEP 39801-243 - SÃƒÆ’Ã†â€™O JACINTO - TEL: (33) 3522-2228  -  CNPJ: 01.014.332/0001-98  ', 1, 'C'); $aux_con[enderecoconsorcio]*/ 
	...
	$this-> SetX(10);
	$this->MultiCell(207, 2,'                    N�mero do documento: '.$codigo_autentificacao, 0, 'L');
}
...
$pdf->MultiCell(187, 8, 'GUIA DE AUTORIZA��O PARA CONSULTA / EXAME ESPECIALIZADO', 1, 'C');
 
$pdf->MultiCell(187, 2, '', 0, 'C');
$pdf->SetFont('Arial','',10);
$pdf->Code39(157,60,"$l[CdSolCons]"); ##IMPRIME O CÃƒâ€œDIGO DE BARRAS
$pdf->SetWidths(array(207));
srand(microtime()*1000000);
$pdf->ln();
 
$pdf->Row2(array('C�DIGO PACOTE: '.$l['idcombo'].'  - PROTOCOLO: '.$l['protocolopac']),1);
$pdf->ln();
...
$pdf->Row2(array('ENDERE�O: '.$l['logradouro'].', N�: '.$l['Numero'].', BAIRRO: '.$l['Bairro'].', CIDADE: '.$l2['NmCidade'].'-MG'),1);
$pdf->ln();
$pdf->Row2(array('DATA DA CONSULTA / EXAME: '.$DtAgCons.'   HOR�RIO '.$HoraAgCons),1);
$pdf->ln();
...
while($aresult = mysqli_fetch_array($sql)){
	$pdf->SetFont('Arial','B',10);
	$pdf->Row(array('PROCEDIMENTO '.$x++.': '.$aresult['cdsus'].'  '.$aresult['NmProcedimento'].' '.$aresult['NmEspecProc'].' - C�DIGO:'.$aresult['CdSolCons']),1);
	$pdf->SetFont('');
	$pdf->SetX(12);
...
$pdf->Ln();
$pdf->Row(array('DECLARO QUE ME FORAM APRESENTADAS AS OP�OES DE LOCAIS PARA A REALIZA��O 		DA CONSULTA/ PROCEDIMENTO/ EXAME DENTRO DOS PRESTADORES CREDENCIADOS JUNTO AO CIESP, 		RESPEITANDO A LOGÃƒï¿½STICA DO MUNICÃƒï¿½PIO.'),1);
/*
$pdf->Ln();
$pdf->Ln();
$pdf->SetX(60);
