...
if(($_GET['id']) && ($_GET['acao']=='edit') || ($_GET['acao']=='del'))
{
	...
	$sql = "SELECT p.is_notifiable as isNotifiable, p.Status as status,CdPaciente,p.CdBairro,NmPaciente,RG,CPF,csus,p.CertidaoMatricula,
...
<script>
	<?php if($row['isNotifiable']== 1) { ?>$("#isNotifiable").prop("checked", true); $("#cel").prop("required", true); <?php }?>
	<?php if($_GET['acao'] == "del") { ?>
