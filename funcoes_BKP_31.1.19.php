...
// ADICIONA AGENDA 
	...
	function lancamentoSemAgenda($CdPaciente,$CdEspecProc,$DtAgCons,$HoraAgCons,$select_forne,$CdPref,$valor,$valorsus,$marcacao=NULL,$Obs=NULL,$Urgente=NULL,$pactuacao=NULL,$retorno=NULL,$remarcado=NULL)
	{
		...
		$qryprof = mysqli_query($db,"SELECT cdprof FROM tbfornespec where CdEspec = '$CdEspecProc' and CdForn = '$select_forne'");
	...
	{
		/*#7840 Solicita estorno para município a qual pertencia a agenda*/
 ...
 */
	...
	//CONSELHO DOS PROFISSIONAIS DE MUNICÍPIO
	...
	function setagatb($cdag, $cdpaciente){
		...
		$sqlag = mysqli_query($db,"SELECT tbagenda_mun.cdagenda_mun,tbagenda_mun.cdunidade,tbagenda_mun.cdprocedimento,tbagenda_mun.obs,
		...
		$sql = mysqli_query($db,"INSERT INTO tbagtab (cdespec,cdunidade,cdprof,cdpref,cdpaciente,dtag,horaag,cdagenda_mun,status,cduserinc,dtinc,hrinc)
	...
	function AtualizaAgenda_mun($cdag,$status){
		...
		if ($status == 'M') {
			$sqlup = mysqli_query($db,"UPDATE tbagenda_mun SET status = 'M' WHERE cdagenda_mun = $cdag");
		}
