...
//consulta solicitacao e agendamento de consultas
        $sql = "SELECT sc.CdSolCons,DtAgCons,HoraAgCons,p.CdPaciente,p.NmPaciente,sc.CdEspec,ac.Valor,
					   ...
					   NmEspecProc,f.NmReduzido,If(proc.NmProcedimento = 'Consulta','',Concat(proc.NmProcedimento,' - ')) as NmProcedimento
                FROM tbsolcons sc INNER JOIN tbpaciente p ON sc.CdPaciente=p.CdPaciente 
								  INNER JOIN tbbairro b ON b.CdBairro=p.CdBairro
								  INNER JOIN tbprefeitura pr ON b.CdPref=pr.CdPref
...
/*
sc.Status = 1 : aguardando
	e ac.Status = 1 : marcado
	  ac.Status = 2: realizado
sc.Status = 2 : cancelado	   
*/
		if (!$showall)
			$sql .= " WHERE sc.Status = '1' AND (ac.Status = '1' OR ac.Status = '2')";
		else
			$sql .= " WHERE sc.Status = '1' AND ac.Status = '2'";
	...
	if ($busca != ""){
		switch ($cbopor){
			...
			case 6: $sql .= " AND pr.NmCidade LIKE '%$busca%'";
					break;	
			case 7: $sql .= " AND NmEspecProc LIKE '%$busca%'";
					break;			
		}
    }
...
?>
	...
	<div id="frm_pesq">
    	<form action="rel_agendamento.php" name="frm_pesq" method="post" id="frm_pesq">
            ...
            &nbsp; por &nbsp;<select name="cbopesq">
                                ...
                                <option value="7" <?php if ($cbopor == 7) echo 'selected="selected"';?> >Nome da Especifica&ccedil;&atilde;o</option>
        ...
        //cor da tabela
         ...
         while($l = mysqli_fetch_array($query)){
               if ($cortb == "#D5F4F4"){
				   ...
				   echo "<td align=\"left\" nowrap=\"nowrap\">&nbsp;$l[NmProcedimento]$l[NmEspecProc]</td>";
				   $total += $l["Valor"];
				   if($l["Valor"] != "")
				   		$l["Valor"] = number_format($l["Valor"],2,",",".");
				   echo "<td align=\"right\">$l[Valor]&nbsp;</td>";
				   echo "</tr>";
				   /*$contador++;
				   $cont++;
				   if ($contador == 5){
