...
<div class="row justify-content-center">
	...
	<div class="col-md-8">
		...
		<?php if($_GET['acao'] != "del") :?>
			...
			<div id="search-error" style="display: none;" class="alert alert-danger" role="alert">
			...
			<div class="row">
				...
				<div class="col-sm-12 col-md-3">
					<div class="form-group">
						...
						<input 
						type="text" 
						class="form-control" onblur="fetchData('cpf',this.value)" value="<?php echo $row["CPF"]; ?>" name="cpf_paciente" id="cpf" 
			...
			<div class="row">
				<div class="col-sm-12 col-md-4">
					...
					<div class="form-group">
						...
						placeholder="Cep" 
						aria-describedby="helpId"/>
						<span id="erroCEP" style="color:red;"></span>
					</div>
			...
			<div class="col-12 text-right">
				...
				<span id="pac_cadastrado" style="color:red;"></span>
			</div>
...
<script type="text/javascript"> 
    function fetchData(filter, value){
		if(!value){
			return;
		}
		$.ajax({
			url: 'api/cadsus/validate.php',
			type: 'POST',
			cache: false,
			data: {
				value : value,
				filter: filter,
				token: <?=json_encode(encrypt(CONNECTION_ID . "%" .  $user_id));?>
			},
			datatype: "json",
			error: function(e) {
				$('#search-error').show(300).text(e.responseText)
				Swal.fire(e.responseText)
			...
			success: function(resposta) {
		    	...
		    	if(typeof data !== 'undefined' && data !== null) {
					...
					if(paciente[keys[0]].controlActProcess.queryAck.queryResponseCode._code === "NF"){
						Swal.fire('error', 'Informações não encontradas', 'error')
						return;
					}
					...
					let patient_name = (paciente.name.given ? paciente.name.given : paciente.name[0].given )
					
					$('#nm_paciente').val(patient_name);
					...
					$('#cep').trigger('blur');
					Swal.fire('Dados do paciente obtidos com sucesso!')
				}
			}    
		}); 
	}
	...
	$('#search').click(function(e){
		...
		if(csus.length === 0){
			Swal.fire('Por favor, digite o número CNS');
			return;
		}
		...
		if (cep != "") {
			...
			if(validacep.test(cep)) {
				...
				$.getJSON("https://viacep.com.br/ws/"+ cep +"/json/?callback=?", function(dados) {
					var nome_pref = $.trim($("#cidade option:selected").text())
					console.log(nome_pref);
					console.log(dados.localidade);
					if (!("erro" in dados)) {
						if(nome_pref.toLowerCase().includes(dados.localidade.toLowerCase()) || (<?php echo $_SESSION['CdTpUsuario'] ?> == 1 || <?php echo $_SESSION['CdOrigem'] ?> == 20) ){
							document.getElementById("btn").disabled = false;
							document.getElementById("erroCEP").innerHTML = "";	
						}else{
							document.getElementById("btn").disabled = true;
							var value = "CEP não pertence ao munícipio!";
		...
		$.ajax({
			...
			success: function(resposta) {
		    	...
		    	if(typeof data !== 'undefined' && data !== null) {
		    		...
		    		var data_paciente = dataano+'-'+datames+'-'+datadia;
					//console.log(data_paciente);
		    		$('#dtnasc_paciente').attr('value', data_paciente);
		    		$('#sexo').val(paciente.administrativeGenderCode._code);
		    		$('#cep').attr('value', paciente.addr.postalCode);
		    		$('#logradouro').attr('value', paciente.addr.streetName);
...
function isDate(dtStr){
	...
	<?php if($row['isNotifiable'] == 0 && isset($row['isNotifiable'])) { ?>$("#isNotifiable").prop("checked", true); $("#cel").prop("required", true); <?php }?>
	<?php if($_GET['acao'] == "del") { ?>
