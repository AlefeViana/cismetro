...
$(document).ready(function() {
  $("form").each(function(){
    $(this).find(':input').not(":button").removeClass().addClass('form-control'); //<-- Should return all input elements in that specific form.
  });
  ...
  $("#commentForm").validate();
    $('#servico').change(function(){  
      $('#class').load('admin/load_class.php?cdserv='+$('#servico').val() , function(response, status, xhr) {
                                            if (status == "error") {
  ...
  $("input[id=valor]").maskMoney({showSymbol:false,decimal:",",thousands:".",allowZero:true,defaultZero:true,allowNegative:true});
  $("input[id=valorsus]").maskMoney({showSymbol:false,decimal:",",thousands:".",allowZero:true,defaultZero:true,allowNegative:true}); 
...
<div class="row justify-content-center">
  <div class="col-md-8">
    <form method="POST" class="form-group" action="<?php echo $destino; ?>" id="commentForm">
   
      <div id="alert"class="alert alert-warning" role="alert"><strong>Atenção</strong> Os Campos com * devem ser preechidos obrigatóriamente </div>
      <fieldset> 
        <h5> Dados do Procedimento </h5>
        <input type="hidden" name="cd_especproc" size="10" readonly value="<?php if(isset($dados["CdEspecProc"])) echo $dados["CdEspecProc"]; else echo "Autom&aacute;tico"; ?>"  />

        <label> C&oacute;digo SUS <input type="text" name="cdsus" id="csus" size="35" value="<?php echo $dados["cdsus"]; ?>"  /></label>
      
        <label> Filiação
          <select name="principal">
            <option value="0" <?php if($dados["principal"] == 0) echo 'selected="selected"'; ?> >Padrão</option>
            <option value="1" <?php if($dados["principal"] == 1) echo 'selected="selected"'; ?> >Principal</option>
          </select>
        </label>
        
        <label class="gr">Especifica&ccedil;&atilde;o: <input name="nm_especproc" type="text" class="required" value="<?php echo $dados["NmEspecProc"];  ?>" size="50" maxlength="100"  /></label>
        ...
        <label>Valor <input type="text" name="valor" id="valor" size="28" class="required"  value="<?php echo  number_format((double)$dados["valor"], 2, ',', ' ');  ?>" /></label>
        
        <label>Valor SUS  <input type="text" name="valorsus" id="valorsus" size="28"  value="<?php echo  number_format((double)$dados["valorsus"], 2, ',', ' ');  ?>"  /></label>  
        ...
        <label> Descri&ccedil;&atilde;o SUS  <input name="desc_sus" type="text" id="desc_sus" value="<?php echo $dados["desc_sus"]; ?>" maxlength="100" /></label>
        ...
        <label> BPA
              <select name="bpa" class="required">
                      <option value=""  > </option>
                      <option value="C" <?php if($dados["bpa"] == "C") echo 'selected="selected"'; ?> >Consolidado</option>
                        <option value="I" <?php if($dados["bpa"] == "I") echo 'selected="selected"'; ?> >Individualizado</option>
      ...
      <label for='nmpreparo'> 
        <span id="contcar" style="font-size:12px;">  </span>
         ...
         <textarea  name="nmpreparo" id='nmpreparo' class="form-control" onkeyup="blocTexto(this.value)"><?php echo $dados['nmpreparo']?></textarea>  
      ...
      <hr>
       <div id="btns" >
          ...
          <?php if($Acao == "edit" || $Acao == "del"){ ?>
                    <input type="button" value="Cancelar" onclick="window.location.href='index.php?i=4'" class="btn btn-lg btn-danger" style="float:right; margin-right:10px;"/>
          <?php } ?>
