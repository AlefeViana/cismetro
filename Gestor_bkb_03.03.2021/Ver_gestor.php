<div id="graphics-app">
      ...
      <div class="graphics">
        <graphic 
          ...
          :key="`graphic-${index}`"
          :url="chart.url"
          :create-chart="chart.createChart"
          :update-chart="chart.updateChart"
          :reload-time="chart.reloadTime"
          :filter="filter"
        ></graphic>
      </div>
    </div>
      ...
      <div class="graphic">
        ...
        <div ref="legend" class="graphic-legend" v-if="legend.length">
          ...
          <div class="menu-legend">
            <div 
              v-for="({ backgroundColor, label, hidden }, index) in legend" 
              :key="`legend-${index}`"
              :class="{ 'legend-hidden': hidden, 'menu-item': true }"
              @click="() => toggleDataSetLegend(index)"
            >
    ...
    <script type="text/x-template" id="graphic-filter">
      <div>
        <div class="main-filter" title="Filtro Principal">
          <div
            v-if="wasNotExcluded('dia')"
            :class="{ selected: filter.main === 'dia' }"
            @click="() => handleMainFilter('dia')"
          >
            Dia
          </div>
          <div
            v-if="wasNotExcluded('semana')"
            :class="{ selected: filter.main === 'semana' }"
            @click="() => handleMainFilter('semana')"
          >
            Semana
          </div>
          <div
            v-if="wasNotExcluded('mes')"
            :class="{ selected: filter.main === 'mes' }"
            @click="() => handleMainFilter('mes')"
          >
            Mês
          </div>
          <div
            v-if="wasNotExcluded('ano')"
            :class="{ selected: filter.main === 'ano' }"
            @click="() => handleMainFilter('ano')"
          >
            Ano
          </div>
            ...
            <form @submit.prevent="handleSubmit">
              ...
              <div class="form-group" v-if="wasNotExcluded('municipio')">
                ...
                <select class="form-control" v-model.number="filter.CdPref">
                  <option
                    v-for="{ CdPref, NmCidade } in options.municipios"
                    :key="'municipio-' + CdPref"
                    :value="CdPref"
                  >
                    {{ NmCidade }}
                  </option>
              ...
              <div class="form-group" v-if="wasNotExcluded('fornecedor')">
                ...
                <select class="form-control" v-model.number="filter.CdForn">
                  <option
                    v-for="{ CdForn, NmForn } in options.fornecedores"
                    :key="'fornecedor-' + CdForn"
                    :value="CdForn"
                  >
                    {{ NmForn }}
                  </option>
              ...
              <div class="form-group" v-if="wasNotExcluded('grupoProcedimento')">
                ...
                <select class="form-control" v-model.number="filter.cdgrupoproc">
                  <option
                    v-for="{ cdgrupoproc, nmgrupoproc } in options.gruposProcedimentos"
                    :key="'grupo-' + cdgrupoproc"
                    :value="cdgrupoproc"
                  >
                    {{ nmgrupoproc }}
                  </option>
              ...
              <div class="form-group" v-if="wasNotExcluded('especificacao')">
                ...
                <select class="form-control" v-model.number="filter.CdEspecProc">
                  <option
                    v-for="{ CdEspecProc, NmEspecProc } in options.especificacoes"
                    :key="'espec-' + CdEspecProc"
                    :value="CdEspecProc"
                  >
                    {{ NmEspecProc }}
                  </option>
              ...
              <div class="form-group" v-if="wasNotExcluded('sexo')">
                ...
                <select class="form-control" v-model.number="filter.Sexo">
                  <option
                    v-for="{ value, text } in options.sexo"
                    :key="'sexo-' + value"
                    :value="value"
                  >
                    {{ text }}
