...
function CalcularIdade($nascimento,$formato,$separador)
{
	...
	$cdevolclinica = $_GET['cdevolclinica'];
	...
	tbestado.NmEstado,
	tbevolclinica.cdevolclinica,
	tbevolclinica.dtinclusao,
	tbevolclinica.obs as EvolucaoClinica,
	tbevolclinica.hruserinc,
	tbevolclinica.`status`
	FROM
	tbevolclinica
	INNER JOIN tbpaciente ON tbevolclinica.CdPaciente = tbpaciente.CdPaciente
	...
	WHERE tbevolclinica.`status` = 'A'
	AND tbevolclinica.cdevolclinica = '$cdevolclinica' ") or die (mysqli_error());
	...
	$pdf->	SetY(39);    
	$pdf->SetFont('Arial','',8);	  
	$pdf->	MultiCell(208, 6,'' ,0,'C');    
	  
	$pdf->	MultiCell(208, 6,'																																												SEXO: '.$lin[Sexo].'										CARTÃO SUS:'.$csus.'								DATA NASCIMENTO: '.FormataDataBr($DtNasc).'  ('.$Idade.')' ,0,'l'); 


	$pdf->	MultiCell(208, 6,'																																												MÃE: '.$lin[NmMae] ,0,'l'); 
	...
	$pdf->	MultiCell(208, 6,'Evolução Clínica' ,1,'C');    
    $pdf->Ln();
	$pdf->SetFont('Arial','',9);	
	$pdf->	MultiCell(208, 5,$lin[EvolucaoClinica] ,0,'L');    
	
	
	
	
	$pdf->SetY(250);	
	$pdf->	MultiCell(208, 5,'ASSINATURA E CARIMBO (PROFISSIONAL DE SAÚDE)' ,1,'C');    
