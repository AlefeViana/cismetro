...
function CalcularIdade($nascimento,$formato,$separador)
{
	...
	$cdreceituario = $_GET['cdreceituario'];
	...
	tbestado.NmEstado,
	tbreceituario.cdreceituario,
	tbreceituario.dtinclusao,
	tbreceituario.obs as Receituario,
	tbreceituario.hruserinc,
	tbreceituario.`status`
	FROM
	tbreceituario
	INNER JOIN tbpaciente ON tbreceituario.CdPaciente = tbpaciente.CdPaciente
	...
	WHERE tbreceituario.`status` = 'A'
	AND tbreceituario.cdreceituario = '$cdreceituario' ") or die (mysqli_error());
...
//	 $pdf->Image($foto[2],5,35,25);
	...
	//$pdf->	MultiCell(208, 6,'																																												SEXO: '.$lin[Sexo].'										CARTÃO SUS:'.$csus.'								RG:'.$RG .'					DATA NASCIMENTO: '.FormataDataBr($DtNasc).'  ('.$Idade.')' ,0,'l'); 
	...
	$pdf->	MultiCell(208, 6,'RECEITUÁRIO' ,1,'C');    
    $pdf->Ln();
	$pdf->	MultiCell(208, 5,$lin[Receituario] ,0,'L');    
