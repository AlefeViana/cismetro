...
class DefaultReport extends jsPDF {
  ...
  show = (action, filename) => {
    ...
    if (![newwindow, newtab, download, thiswindow].includes(action = action?.toString().toLowerCase()))
      action = newwindow;
    ...
    if (filename || action === download)
      filename = (filename ?? `${[this.#customOptions.title, this.#customOptions.subtitle].filter(e => e).join('_') || 'relatorio'}_:timestamp:`)
        ...
        .replace(/_?(?:[\\/:*?"<>|]+_?)+/g, '_');
    const uri = (action == newwindow || action == newtab || action == thiswindow) && (this.output('bloburi'));
  ...
  constructor(jsPdfOptions, customOptions) {
    ...
    [customOptions.headerBG, customOptions.logo].forEach(e => {
      if (e && e.imageData)
        this.addImage(e.imageData, e.format, e.x, e.y, e.width, e.height, e.alias, e.compression, e.rotation);
    });

    this.setFont('Proxima Nova Font', 'normal').setTextColor(customOptions.headerFontColor);
    if (customOptions.organization)
      this.setFontSize(12)
        .text(customOptions.organization, this.internal.pageSize.getWidth() / 2, 15, { align: 'center' });
    if (customOptions.contactInfo) {
      ...
      customOptions.contactInfo.forEach((e, i) => this.text(e, this.internal.pageSize.getWidth() / 2, 19 + i * 4, { align: 'center' }));
    }
    const rowDist = 5;
    const start = 44;
