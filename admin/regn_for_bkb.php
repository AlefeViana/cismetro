...
else
{
	if (substr($CdForn,0,4) == "Auto")
	{
		require("../conecta.php");
		//gera um novo codigo
		/* $qry = mysqli_query($db,"SELECT MAX(CdForn) FROM tbfornecedor") 
				or die(TrataErro(mysqli_errno(),'','../index.php?p=lista_for','regn_for:gerar novo codigo'));
		$CdForn = mysqli_result($qry,0) + 1; */
		//inclui fornecedor
		$sql = "INSERT INTO tbfornecedor (NmForn,NmReduzido,IE,CNPJ,CNES,cpf,cns,Telefone,Fax,NmResp,TelResp,Email,Logradouro,Numero,Compl,Bairro,CEP,CdCidade,UserInc,tpforn)			                 VALUES('$NmForn','$NmReduzido','$IE','$CNPJ','$CNES','$cpf','$cns','$Tel','$Fax','$NmResp','$TelResp','$Email','$Logr','$Num','$Compl','$Bairro','$Cep',$CdCidade,$_SESSION[CdUsuario],'$tpforn')";	
