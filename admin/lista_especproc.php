...
//consulta
        $sql = "SELECT CdEspecProc, NmEspecProc, NmProcedimento, e.Status
                FROM tbespecproc e INNER JOIN tbprocedimento p ON e.CdProcedimento=p.CdProcedimento
				WHERE CdEspecProc > 1";
    if ($busca != ""){
		switch ($cbopor){
			case 1: $sql .= " AND CdEspecProc = $busca";
					break;
			case 2: $sql .= " AND NmEspecProc LIKE '$busca%'";
					break;
		}
    }
    ...
    //echo $sql;
//executa a consulta
    $query = mysqli_query($db,$sql)or die (TrataErro(mysqli_errno(),'','index.php?p=inicial','lista_especproc:qtd regs'));
...
// Executa a query no MySQL com o limite de linhas.
    $limsql = $sql." ORDER BY NmProcedimento,NmEspecProc LIMIT $inicio, $lpp ";
    $query = mysqli_query($db,$limsql)or die(TrataErro(mysqli_errno(),'','index.php?p=inicial','lista_especproc:consulta dados'));
//inicio do form de pesquisa
?>
	<div id="rotina" style="text-align:center">
    	Cadastro de Exames e Procedimentos Cir&uacute;rgicos<br /><br />
    </div>
	<div id="frm_pesq">
    	<form action="index.php?p=lista_especproc" method="post">
            ...
            &nbsp; por &nbsp;<select name="cbopesq">
                                ...
                                <option value="2" <?php if ($cbopor == 2 || $cbopor == "") echo 'selected="selected"';?> >Especifica&ccedil;&atilde;o</option>
            				 </select>	
            <input type="submit" value="Pesquisar" name="btnpesq" />                 
            &nbsp;&nbsp;
			<input type="button" name="BtnCad" value="Cadastrar" onClick="javascript:window.location.href='index.php?p=frm_cadespecproc'">
        </form>
    </div>
<?php
...
//inicio tabela conteudo
      echo '<center><table width="98%" border="0" id="grid">';
        ...
        //cor da tabela
         ...
         while($l = mysqli_fetch_array($query)){
               if ($cortb == "linha2"){
                   ...
                   $cortb = "linha2";
			   	   ...
			   	   $link = 'index.php?p=frm_cadespecproc&id='.$l[CdEspecProc].'&acao=edit';
			   	   $link_del = 'index.php?p=frm_cadespecproc&id='.$l[CdEspecProc].'&acao=del';
                   echo "<tr class=".$cortb.">";
				   ...
				   echo "<td>$l[CdEspecProc]</td>";
                   echo "<td align=\"left\">$l[NmEspecProc]</td>";
				   echo "<td align=\"left\">$l[NmProcedimento]</td>";
         ...
         $param = "&p=lista_especproc&pesq=$busca&cbopesq=$cbopor";
		 if ($pags > 1)
         ...
         if($pag < ($pags - 1)) {
          ...
          echo '<h3><center><font face="verdana","arial" color="FF6464">Nenhuma especifica&ccedil;&atilde;o encontrada</font></center></h3>';
	  }
