<?php
	...
	$cota = mysqli_query($db,"SELECT cdcota FROM tbcota 
						INNER JOIN tbgenfat ON tbcota.idgenfat = tbgenfat.idgenfat
						WHERE CdEspecProc = $cdespecificacao 
						AND (MONTH(tbgenfat.dtini) = $mesaux AND MONTH(tbgenfat.dtfim) = $mesaux)
						AND (YEAR(tbgenfat.dtini) = $year AND YEAR(tbgenfat.dtfim) = $year)");
	$cdcota = mysqli_fetch_array($cota);

	$qtdres = getCotaGastas($cdcota['cdcota'],$_SESSION['CdOrigem']);
