...
$cdfornespera 	= $_POST["cd_fornecedor"] ?? 0;//Cod. fornecedor preferencial Fila de Espera
...
$pactuacao 		= (int) $_POST["pactuacao"];
$retorno    	=  (int)$_POST["retorno"];
$remarcado  	=  (int)$_POST["remarcado"];

$veterinaria  	=  $_POST["veterinaria"];
$cdanimal  		=  ($veterinaria)? (int)$_POST["cdanimal"] : 'null';
...
$hora	 	= $_POST["hora"];*/
	...
	if(isset($_POST["cd_especcis"])){
		...
		{	
			...
			$dtesp = date('Y-m-d');
			$hresp = date('H:i:s');
			$datasol = $data;//Transforma a data de solicitação do médico
			$status = 'E';

			$sql = "INSERT INTO tbsolcons (cdanimal,CdSolCons,CdPaciente,CdEspecProc,CdUsuario, Status, Protocolo,Obs1,Urgente, pactuacao,retorno,remarcado,dtinc,userinc,hrinc,CdPref,cdmed, useresp, dtesp, hresp, CdUnid, dtmedsol, cdfornespera, CdTipo,TpAgen,idcombo)
					VALUES ($cdanimal,'$CdSolCons','$CdPaciente','$CdEspeccis[$n]','$CdUser','$status','$Protocolo','$Obscis[$n]','$Urgentecis[$n]', '$pactuacao','$retorno','$remarcado','$dtinc','$userinc','$hrinc','$CdPref',$cdmed, '$useresp', '$dtesp', '$hresp', '$cdunid', '$datasol', $cdfornespera, '$CdTipo','$con',$idcombo)";
			//var_dump($sql); die();
