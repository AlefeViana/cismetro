<?php
	...
	$sql = mysqli_query($db,"SELECT af.cbo, epc.nmespecialidade,af.obs1 as obsmun, sc.dtm, sc.hrm, af.obs2 as obspac , af.cdendc,pe.Logradouro, pe.Telefone as TelProf ,pe.Numero as crednumero,pe.Bairro as bairrocred,
							 pe.Cidade,pe.Estado,sc.CdSolCons,DtAgCons,HoraAgCons, p.CdPaciente,p.RG, p.NmMae, f.CdForn,f.Bairro,f.logradouro, f.Numero, f.CdCidade, ep.nmpreparo, 
							 p.NmPaciente, ac.Valor, p.DtNasc, ac.CdUsuario, u.Login, u.NmUsuario, ac.CdForn, pr.NmCidade,sc.Protocolo, sc.DtInc,pr.CdPref, ep.CdEspecProc, ac.qts, ac.protocolopac, 
							 NmEspecProc, f.NmForn,sc.Status, ac.Status as StatusAg,Urgente, f.NmReduzido, f.cns,sc.Obs, ac.obs,ep.cdsus, Pa.NmProcedimento, sc.Obs1, p.DtNasc, p.csus, 
							 f.Compl, p.Celular, prof.CdProf as cdprof, prof.Nome as nmprof,prof.NumCPP, und.NmForn as NmUnidade, und.NmReduzido as NmRedUnidade, und.CdForn as CdUnidade, f.Telefone
	...
	$Nmusuario_agendador = $l['NmUsuario'];
	$nmprof = $l['nmprof'];
	$NumCPP = $l['NumCPP'];
	$dtm = FormataDataBR($l['dtm']);
	$hrm = $l['hrm'];
...
var  $i=0;
   ...
   function Footer() {
	...
	$this ->MultiCell(207, 6, $dadoscis['Titulo'].' - '.$dadoscis['Nome'].' - '.$dadoscis['Cidade'].'/'.$dadoscis['Estado'].'
	          CNPJ:19.947.645/0001-64'.$dadoscis['CNPJ'], 1, 'C');
	...
	$this-> SetY(273);
	$this->MultiCell(207,5,'                    Emitido por : '.mb_strtoupper($nomeUsuer).' - '.$data_agora, 0, 'L');
	$this-> SetX(10);
	$this->MultiCell(207,5,'                    Agendado por : '.$Nmusuario_agendador.' - '.$dtm.' '.$hrm, 0, 'L');
	$this-> SetX(10);
	$this->MultiCell(207,5,'                    Executado por : '.$nmprof.' - CRM :'.$NumCPP, 0, 'L');
	$this-> SetX(10);
	$this->MultiCell(207,5,'                    Este documento foi assinado eletronicamente. Número do documento : '.$codigo_autentificacao, 0, 'L');
...
//apontamos a fonte que serï¿½ utilizada no texto
	...
	$pdf->ln();
	$pdf-> SetX(2);
	$pdf->Row(array('CBO: '.$l['cbo'].' - '.$l['nmespecialidade'] ),1);
	$pdf->ln();
	$pdf-> SetX(2);
	$pdf->SetFont('Arial','B',10);
	...
	$pdf->Row(array('Observação ao paciente: '.$l['obspac'] ),1);
	$pdf->ln();
	$pdf-> SetX(2);
