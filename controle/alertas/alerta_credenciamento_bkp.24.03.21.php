...
if(isset($_SESSION['CdTpUsuario']) && $_SESSION['CdTpUsuario'] == 1){
    // Novos Credenciados
    $val_alerta = 0;
    $novos_cred =  $busca_alertas." AND ( fl.andamento = 0 and a.Cdanx is NULL) GROUP BY fl.CdForn ORDER BY f.NmForn ";
    //var_dump($novos_cred); die();
    $novos_credenciados = mysqli_query($db, $novos_cred);
    $forn_list = "";
    ...
    $aprova_cred =  $busca_alertas." AND ( fl.andamento = 0 and a.Cdanx > 0 ) GROUP BY fl.CdForn ORDER BY f.NmForn ";
    //var_dump($novos_cred); die();
