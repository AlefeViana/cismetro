...
$val_alerta = 0;
if ($_SESSION['cdunidade'] > 0){
    $unidade = " AND fornm.CdForn =".$_SESSION['cdunidade'];
}

if(isset($_SESSION['CdTpUsuario']) && $_SESSION['CdTpUsuario'] == 1){

    $sql_nao_conf = "   SELECT sc.CdSolCons from tbsolcons sc
    ...
    if(mysqli_num_rows($sql_nao_conf) > 0){                   
        $texto .= "<li class='list-group-item d-flex justify-content-between align-items-center'>Novos agendamentos realizados confira!<i class='fas fa-caret-square-right irFP' alt='Ir para página!' style='color:orange;'></i></li>";
        $val_alerta +=1;
    }
    ...
    $sql_nao_impresso = "   SELECT sc.CdSolCons from tbsolcons sc
                        INNER JOIN tbagendacons ac on ac.CdSolCons = sc.CdSolCons
    ...
    $sql_paciente_solicitados = "SELECT COUNT(sc.CdSolCons) as qts 
                         from tbsolcons sc
                         WHERE (sc.`Status` = 'E' )";
    $sql_paciente_solicitados = mysqli_query($db, $sql_paciente_solicitados);
    if(mysqli_num_rows($sql_paciente_solicitados) > 0){    
        $qts_solicitacao = mysqli_fetch_array($sql_paciente_solicitados);               
        $texto .= "<li class='list-group-item d-flex justify-content-between align-items-center'>Existe ".$qts_solicitacao['qts']." paciente(s) solicitados para agendamento!<i class='fas fa-caret-square-right irAFE' alt='Ir para página!' style='color:orange;'></i></li>";
        $val_alerta +=1;
    }
    ...
    $sql_nao_conf = "   SELECT sc.CdSolCons from tbsolcons sc
                        INNER JOIN tbagendacons ac on ac.CdSolCons = sc.CdSolCons
                        LEFT JOIN tbfornecedor_mun as fornm on fornm.CdForn = sc.CdUnid
                        WHERE (sc.`Status` = 1 and ac.`Status` = 1 )
                         and sc.CdPref = $_SESSION[CdOrigem] and ac.DtAgCons <= CURDATE() $unidade";
    $sql_nao_conf = mysqli_query($db, $sql_nao_conf);
    if(mysqli_num_rows($sql_nao_conf) > 0){                   
        $texto .= "<li class='list-group-item d-flex justify-content-between align-items-center'>Novos agendamentos realizados confira!<i class='fas fa-caret-square-right irFP' alt='Ir para página!' style='color:orange;'></i></li>";
        $val_alerta += 1;
    ...
    $sql_nao_impresso = "  SELECT sc.CdSolCons from tbsolcons sc
                        INNER JOIN tbagendacons ac on ac.CdSolCons = sc.CdSolCons
                        LEFT JOIN tbfornecedor_mun as fornm on fornm.CdForn = sc.CdUnid
                        WHERE (sc.`Status` = 1 and ac.`Status` = 1 ) and sc.CdPref = $_SESSION[CdOrigem] 
                        and ac.DtAgCons >= CURDATE() and sc.impresso = 'N' $unidade";
    $sql_nao_impresso = mysqli_query($db, $sql_nao_impresso);
    if(mysqli_num_rows($sql_nao_impresso) > 0){                   
        $texto .= "<li class='list-group-item d-flex justify-content-between align-items-center'>Existem guias a serem impressas!<i class='fas fa-caret-square-right irFP-imp' alt='Ir para página!' style='color:orange;'></i></li>";
        $val_alerta += 1;
