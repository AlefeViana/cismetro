...
var tipo = '';
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			dayClick: function(date, allDay, jsEvent, view) {
				...
				<?php
					/* if($_SESSION['TpUsuario']!='Profissional de Saúde'){
						
							echo "window.location.href='?i=18a&data='+dia+'/'+mes+'/'+ano;";		 
	...
	{
	$fornecedor_03  =  $_SESSION["cdfornecedor"];
	$forn = " WHERE f.CdForn = '$fornecedor_03' ";
				...
				?>	
					...
					// window.location.href='?i=18&data='+dia+'/'+mes+'/'+ano+'&hora='+hora+':'+minuto;
				}   
	...
	function pesquisa(valor,tipo){
			...
			if (val == 1){
				$('#calendar').fullCalendar('removeEventSource','load_eventostfd.php?pesq='+aux+'&cbopesq='+auxcbo);						
			}
			else{
				$('#calendar').fullCalendar('removeEventSource','load_eventostfd.php');
				val = 1;					
			}
							
			$('#calendar').fullCalendar('addEventSource', 'load_eventostfd.php?pesq='+valor+'&cbopesq='+tipo); 	
...
<h1>Controle &raquo; Calend&aacute;rio </h1>
<!-- 		
<!--    <div style="text-align:left"></div>
    <div id="frm_pesq" style="clear:both; height:50px;" >  
    
        <input type="text" name="pesq" id="pesq" style=" float:left; padding:8px; border:#CCCCCC solid 1px; width:200px; font-style:italic; background:url(img/icon_lupa.jpg) no-repeat; padding-left:25px; "  /> 
