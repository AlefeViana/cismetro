...
$pdf->SetLeftMargin('10');
	...
	$pdf->Sety(11); 
	$pdf->SetX(52);  $pdf->SetFont('Arial','B',8);
	$pdf->MultiCell(115,6, 'ENDEREÇO: '.$end[Logradouro].', Nº: '.$end[Numero].', BAIRRO: '.$end[Bairro].', CIDADE: '.$end[NmCidade].'-MG',0, 'C');
	$pdf->SetFont('Arial','B',8);
	$pdf->Sety(14); 
	$pdf->SetX(52);  
	...
	$pdf-> SetY(5);	
	$pdf-> SetX(169);	
	$pdf->	MultiCell(40, 6,'Data Emissão: '.$data.' Hora Emissão: '.$horario.'',1);    
	$pdf->SetY(30);
...
//apontamos a fonte que será utilizada no texto
	...
	$pdf->Row2(array('PACIENTE: '.strtoupper($l[NmPaciente])."   CIH: ".$l[CdPaciente]."   CARTÃO SUS: ". $l[csus]),1);
	$pdf->ln(0.2);
	$pdf->SetX(2);
	...
	$pdf->MultiCell(207, 3, 'Secretário Muncipal de Saúde ou Prefeito Municipal', 0, 'C');
	$pdf->ln();
	$pdf->MultiCell(207, 10, 'Declaro que realizei o exame, na data e clínica, conforme acima agendado.', 0, 'C');
	$pdf->MultiCell(207, 5, '_____________________________________________', 0, 'C');
	$pdf->MultiCell(207, 3, 'Assinatura do Beneficiário', 0, 'C');
	...
	if(mysqli_num_rows($query_forn) > 0)
		...
		{	
			...
			srand(microtime()*1000000);
			$pdf-> SetX(2);
			$pdf->Row(array('PACIENTE: '.strtoupper($l[NmPaciente])."   CIH: ".$l[CdPaciente]."   CARTÃO SUS: ". $l[csus]),1);
			$pdf->ln(0.2);
			$pdf-> SetX(2);
			...
			$pdf-> SetX(2);
				$pdf->Row(array('ENDEREÇO: '.$n_forn[Logradouro].', Nº: '.$n_forn[Numero].', BAIRRO: '.$n_forn[Bairro].', CIDADE: '.$n_forn[NmCidade].'-MG'),1);
			...
			while($n_proc = mysqli_fetch_array($sql5))
			{
		    	...
		    	$pdf->Row(array('PROCEDIMENTO '.$s.': '.$cdsus.'  '.$NmProcedimento.' '.$NmEspecProc.'  CÓDIGO: '.$n_proc[CdSolCons].'   PROTOCOLO: '.$n_proc[protocolopac]),1);
			...
			$pdf->Row(array('DATA DA CONSULTA / EXAME: '.$DtAgCons.'   HORÁRIO '.$HoraAgCons.''.$aux_hora),1);
			$pdf->ln(0.2);
			$pdf-> SetX(2);
			/*
			if($l[sit] == 'I')
			...
			if ($l[Obs] != ''){
				...
				$pdf-> SetX(2);
				$pdf->Row(array('OBSERVAÇÕES FORNECEDOR:  '.$of[obspac]),1);
				$pdf->ln(0.2);
