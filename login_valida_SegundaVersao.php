...
// Se não houver submissão de formulário, o processo de autenticação é realizado
    ...
    if ($correct_credentials) {
        ...
        // Cria uma string com a data e hora atual formatada
        $dataValida = $ano . '-' . $mes . '-' . $dia . ' ' . $horas;

        // Verifica se o usuário está autenticado em outra máquina confiável
        $sql_atuenticacao = "   SELECT  ipuser, DtInc
                                FROM    tbmaquinas_confiaveis
                                WHERE   CdUsuario = '$userId' 
                                AND     DtInc > '$dataValida'
                                AND     `status`  = 1";
        ...
        // Verifica se o endereço IP atual consta na lista de máquinas confiáveis
