...
function NbLines($w,$txt)
{
	...
	$sql = mysqli_query($db,"SELECT af.cbo,af.obs1 as obsmun, af.obs2 as obspac ,vet.cdanimal, vet.nome as nmpet, vet.cor, vr.nmraca,af.cdendc, f.Telefone, pe.Telefone as TelProf,pe.Logradouro,pe.Numero as crednumero,
	pe.Bairro as bairrocred,pe.Cidade,pe.Estado, sc.CdSolCons,DtAgCons,HoraAgCons, p.CdPaciente,p.RG, p.NmMae, f.CdForn,f.Bairro,
	f.logradouro, f.Numero, f.CdCidade, ep.nmpreparo, fe.preparo, p.NmPaciente, ac.Valor, p.DtNasc, ac.CdUsuario, u.Login, ac.CdForn, pr.NmCidade,
	sc.Protocolo, sc.DtInc,pr.CdPref, ep.CdEspecProc, ac.qts, ac.protocolopac, NmEspecProc, f.NmForn,sc.Status, ac.Status as StatusAg,Urgente,
	NmReduzido,sc.Obs, ac.obs,ep.cdsus, Pa.NmProcedimento, sc.Obs1, p.DtNasc, p.csus, sc.idcombo,p.Celular
	...
	INNER JOIN tbfornespec fe ON fe.CdForn = ac.CdForn AND fe.CdEspec = sc.CdEspecProc AND fe.cdprof = ac.cdprof
...
if($l['cdendc'] > 0){
	...
	$pdf->ln();
	$pdf->Row(array('Cidade: '.S::create($l['Cidade'])->titleize(["de", "da", "do"]).' - SP'),1);
}else{
...
// $pdf->ln();
$pdf->Row(array('Observação ao agendador: '.$l['obsmun'] ),1);
$pdf->ln();
...
while($aresult = mysqli_fetch_array($sql)){
	$DtAgCons = $aresult['DtAgCons'];
	$DtAgCons = explode('-',$DtAgCons);
	$DtAgCons  = $DtAgCons[2].'/'.$DtAgCons[1].'/'.$DtAgCons[0];
	$HoraAgCons = $aresult['HoraAgCons'];
	$pdf->SetFont('Arial','B',10);
	$pdf->Row(array('PROCEDIMENTO '.$x++.': '.$aresult['cdsus'].'  '.$aresult['NmProcedimento'].' '.$aresult['NmEspecProc'].' - CÓDIGO:'.$aresult['CdSolCons'].' - CBO:'.$aresult['cbo']),1);
	$pdf->Row2(array('DATA DA CONSULTA / EXAME: '.$DtAgCons.'   HORÁRIO '.$HoraAgCons),1);
	// $pdf->SetFont('');
	$pdf->SetX(12);
	$pdf->MultiCell(180, 4,'PREPARO: '.rtrim(ltrim($aresult['preparo'])), 0, 'L');
