...
function calendario(){
    ...
    $retorno.= "<th colspan='2' align='center'>Dia Cópia Agendas: <input type='text' id='dia_inicio' name='dia_inicio' size='13'></th><th colspan='2' aling='left'><div id='btns'><a href='javascript: void(0);' onclick='javascript: replicar_todas();' ><input type='button' name='check_replicar' value='Replicar Agendas'></a></div>
				<th>
					<div id='btns'>
	...
	for ($j=1; $j<=$ultimo_dia; $j++){
		...
		$retorno.= "<td width='23%'><select id='cmb_per[$j]' name='cmb_per[$j]' style='height:30px;' $dis><option value=''>Selecione...</option><option value='M' $selectm>Manhã</option><option value='T' $selectt>Tarde</option></select></td>";
		...
		elseif($ld[periodo2] == "T")
			$selectt = "selected='selected'";			
		$retorno.= "<td width='23%'><select id='cmb_per2[$j]' name='cmb_per2[$j]' style='height:30px;' $dis><option value=''>Selecione...</option><option value='M' $selectm>Manhã</option><option value='T' $selectt>Tarde</option></select></td>";
