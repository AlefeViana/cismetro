...
// ADICIONA AGENDA 
	...
	function getCompetencia($data, $vigencia = NULL)
	{
		if($vigencia == 'M'){
			...
			$query = " SELECT idgenfat FROM tbgenfat WHERE dtini = '01-01-{$data}' and dtfim = '31-12-{$data}'";
		}else{
			$query = " SELECT tbgenfat.idgenfat FROM tbgenfat WHERE '{$data}' BETWEEN tbgenfat.dtini AND tbgenfat.dtfim ";
		}
 ...
 */
	...
	*/
	function contratoCerto($CdEspec,$CdFornecedor=NULL,$data=NULL)
	{
		...
		$retorno = readJoin('cont.CdContrato,cont.Valor,cont.DtValidade,cont.DtValidadef,cont.CdForn',
	...
	*/
	...
	{
		$retorno = readJoin('conte.CdEspecProc,esp.NmEspecProc',
							'tbcontratoespec AS conte',
